    ...
    for i, module_def in enumerate(module_defs):
        ...
        if module_def['type'] == 'convolutional':
            ...
            if batch_normalize:
                ...
                modules.add_module('conv_with_bn_%d' % i, conv)
                bn = nn.BatchNorm2d(filters)
                modules.add_module('batch_norm_%d' % i, bn)
            else:
                # 增加卷积层
                conv = nn.Conv2d(in_channels=output_filters[-1],
                                 out_channels=filters,
                                 ...
                                 padding=pad,
                                 bias=not batch_normalize)
                modules.add_module('conv_without_bn_%d' % i, conv)
            # 判断激活曾是否为Leaky，是则增加。
            if activation == 'leaky':
                modules.add_module('leaky%d' % i, nn.LeakyReLU(0.1))
        ...
        #     modules.add_module('shortcut_%d' % i, EmptyLayer())
            ...
            modules.add_module('route_%d' % i, Route(layers))

        elif module_def['type'] == 'shortcut':
            froms = int(module_def['from'])
            filters = output_filters[int(module_def['from'])]
            modules.add_module('shortcut_%d' % i, ShortcutLayer(froms))
